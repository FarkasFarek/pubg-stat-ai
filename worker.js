...
<div class="overlay"><div class="shell">
  <aside class="panel left">
    ...
    <div class="eg">
      ...
      <div class="mc"><div class="ml">Top 10</div><div class="mv" id="top10">0</div></div>
    </div>
  </aside>
  ...
  <aside class="panel right">
    ...
    <div class="status">
      ...
      <div class="r2">
        ...
        <div class="card"><div class="cl">Meccsek</div><div class="cv" id="matches" style="font-size:1.6rem">0</div></div>
      </div>
      <div class="card"><div class="cl">Adatforr\u00e1s</div><div class="cv" id="source" style="font-size:1rem;color:#4f98a3">--</div><div class="bar"><span id="wbar"></span></div><div class="cs" id="usub">V\u00e1rakoz\u00e1s...</div></div>
    </div>
  </aside>
</div></div>
<section class="controls">
  <h1>PUBG Live Overlay</h1>
  <p>Stream el\u0151tt rejtsd el ezt a panelt a <strong>H</strong> billenty\u0171vel.</p>
  <div class="field"><label>PUBG j\u00e1t\u00e9kosn\u00e9v</label><input id="iname" placeholder="pl. FarkasFarek"></div>
  <div class="g2">
    <div class="field"><label>Platform</label><select id="iplat"><option value="steam">Steam (PC)</option><option value="xbox">Xbox</option><option value="psn">PlayStation</option></select></div>
  ...
  <div class="hint">Adatforr\u00e1s: pubg.op.gg (API-kulcs n\u00e9lk\u00fcl)</div>
</section>
<script>
  const S={kills:0,kd:0,wins:0,damage:0,hs:0,assists:0,top10:0,matches:0,winRatio:'0.0',pname:'--',source:'--',loop:null,mode:'Idle',prevKills:0};
  const $=id=>document.getElementById(id);
  const E={mode:$('mode'),kills:$('kills'),kd:$('kd'),wins:$('wins'),damage:$('damage'),hs:$('hs'),assists:$('assists'),top10:$('top10'),matches:$('matches'),winpct:$('winpct'),wbar:$('wbar'),pname:$('pname'),source:$('source'),ubadge:$('ubadge'),usub:$('usub'),toast:$('toast'),tico:$('tico'),ttitle:$('ttitle'),tsub:$('tsub'),tcount:$('tcount')};
  const fmt=n=>n>=1e6?(n/1e6).toFixed(1)+'M':n>=1e3?(n/1e3).toFixed(1)+'K':String(Math.round(n));
  function render(){E.mode.textContent=S.mode;E.kills.textContent=fmt(S.kills);E.kd.textContent=Number(S.kd).toFixed(2);E.wins.textContent=fmt(S.wins);E.damage.textContent=fmt(S.damage);E.hs.textContent=fmt(S.hs);E.assists.textContent=fmt(S.assists);E.top10.textContent=fmt(S.top10);E.matches.textContent=fmt(S.matches);E.winpct.textContent=S.winRatio+'%';E.pname.textContent=S.pname;E.source.textContent=S.source;E.wbar.style.width=Math.min(100,parseFloat(S.winRatio))+'%';const t=new Date().toLocaleTimeString('hu-HU');E.ubadge.textContent=t;E.usub.textContent='Friss\u00edtve: '+t;}
  function toast(diff,total){E.tico.textContent='+'+diff;E.ttitle.textContent=diff===1?'\u00daj kill!':diff+' \u00faj kill!';E.tsub.textContent=new Date().toLocaleTimeString('hu-HU');E.tcount.textContent=total;E.toast.classList.add('show');clearTimeout(toast.t);toast.t=setTimeout(()=>E.toast.classList.remove('show'),3400);}
  async function tick(){S.mode='Live';render();const name=$('iname').value.trim();const plat=$('iplat').value;if(!name)throw new Error('Add meg a j\u00e1t\u00e9kosnevedet.');const r=await fetch('/api/stats?name='+encodeURIComponent(name)+'&platform='+plat);if(!r.ok){const e=await r.json().catch(()=>({}));throw new Error(e.error||'API hiba '+r.status);}const d=await r.json();const s=d.stats;const pk=S.kills;S.pname=s.name;S.kills=s.kills;S.kd=s.kd;S.wins=s.wins;S.damage=s.damage;S.hs=s.headshots;S.assists=s.assists;S.top10=s.top10;S.matches=s.matches;S.winRatio=s.winRatio;S.source=d.source||'op.gg';if(pk>0&&s.kills>pk)toast(s.kills-pk,s.kills);S.prevKills=s.kills;render();}
  function startLive(){stopLoop();const sec=Math.max(10,Number($('iint').value||30));tick().catch(handleErr);S.loop=setInterval(()=>tick().catch(handleErr),sec*1000);}
  function startDemo(){stopLoop();S.pname=$('iname').value.trim()||'DemoPlayer';S.kills=1240;S.kd=3.42;S.wins=87;S.damage=312500;S.hs=380;S.assists=210;S.top10=195;S.matches=520;S.winRatio='16.7';S.source='Demo';S.mode='Demo';render();S.loop=setInterval(()=>{S.kills+=Math.floor(Math.random()*3);S.damage+=Math.floor(Math.random()*400);if(Math.random()>.7)S.wins++;S.matches++;S.kd=(S.kills/Math.max(1,S.matches*.6)).toFixed(2);S.winRatio=((S.wins/S.matches)*100).toFixed(1);const d=Math.floor(Math.random()*3);if(d>0)toast(d,S.kills);render();},3000);}
...
export default {
  async fetch(request, env) {
    ...
    // /api/stats?name=FarkasFarek&platform=steam
    if (url.pathname === '/api/stats') {
      const name     = url.searchParams.get('name');
      const platform = url.searchParams.get('platform') || 'steam';
      if (!name) return json({ error: 'name parameter szukseges' }, 400);

      // op.gg kereso: eloszor megkeressuk a jatekos adatait
      const searchUrl = `https://pubg.op.gg/api/users/search?nickname=${encodeURIComponent(name)}&server=${platform}`;

      let searchRes;
      try {
        searchRes = await fetch(searchUrl, {
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; PUBG-Overlay/1.0)',
            'Accept': 'application/json',
            'Referer': 'https://pubg.op.gg/',
          },
        });
      } catch (e) {
        return json({ error: 'op.gg elerhetetetlen: ' + e.message }, 502);
      }

      if (!searchRes.ok) {
        return json({ error: `op.gg hiba ${searchRes.status}` }, searchRes.status);
      }

      let searchData;
      try { searchData = await searchRes.json(); }
      catch { return json({ error: 'op.gg valasz nem parselhato' }, 502); }

      // Keressuk ki a legjobb egyezest
      const users = searchData?.data || searchData?.users || [];
      if (!users.length) return json({ error: `Nem talalhato jatekos: ${name}` }, 404);

      const user = users.find(u =>
        (u.nickname || u.name || '').toLowerCase() === name.toLowerCase()
      ) || users[0];

      const statsRaw = user?.stats || user?.rankedStats || {};

      // Normalizealjuk a statokat
      const kills    = statsRaw.kills      ?? statsRaw.Kills      ?? 0;
      const wins     = statsRaw.wins       ?? statsRaw.Wins       ?? 0;
      const matches  = statsRaw.games      ?? statsRaw.roundsPlayed ?? statsRaw.matches ?? 1;
      const damage   = statsRaw.damageDealt ?? statsRaw.damage    ?? 0;
      const headshots= statsRaw.headshotKills ?? statsRaw.headshots ?? 0;
      const assists  = statsRaw.assists    ?? 0;
      const top10    = statsRaw.top10s     ?? statsRaw.top10      ?? 0;
      const kd       = statsRaw.kda        ?? statsRaw.kd         ?? (kills / Math.max(1, matches - wins));

      return json({
        ok: true,
        source: 'pubg.op.gg',
        stats: {
          name:     user.nickname || user.name || name,
          kills:    Math.round(kills),
          kd:       Number(kd).toFixed(2),
          wins:     Math.round(wins),
          damage:   Math.round(damage),
          headshots:Math.round(headshots),
          assists:  Math.round(assists),
          top10:    Math.round(top10),
          matches:  Math.round(matches),
          winRatio: matches > 0 ? ((wins / matches) * 100).toFixed(1) : '0.0',
        },
      });
    }
